...
import { getWordsListByLessonId, getLessonByLessonId } from '@/services/bookService'
import {
  ArrowLeftIcon,
  SpeakerWaveIcon,
  BookOpenIcon,
...
const conversationData = ref({
  ...
  lines: [
    ...
    },
    {
      ...
      translation: 'I am Kim. I came from Korea.',
      audioTime: 18.2,
    },
    {
      id: 4,
      speaker: 'B',
      parts: [t('そうですか。'), t('学生', 'がくせい'), t('ですか。')],
      translation: 'Is that so? Are you a student?',
      audioTime: 23.0,
    },
    {
      id: 5,
      ...
      translation: 'Yes, I am studying Japanese at university.',
      audioTime: 26.5,
    },
  ],
})
...
const examplesData = ref([
  {
    id: 1,
    parts: [t('私', 'わたし'), t('は'), t('学生', 'がくせい'), t('です。')],
    translation: 'I am a student.',
    audioTime: 32.0,
  },
  {
    id: 2,
    parts: [t('彼', 'かれ'), t('は'), t('先生', 'せんせい'), t('ではありません。')],
    translation: 'He is not a teacher.',
    audioTime: 36.5,
  },
  {
    id: 3,
    parts: [t('あの'), t('人', 'ひと'), t('は'), t('誰', 'だれ'), t('ですか。')],
  ...
  <div class="min-h-screen pt-0 pb-48 bg-gray-50 flex flex-col">
    ...
    <div class="flex flex-1 px-6">
      ...
      <aside class="fixed left-6 top-32 w-12 flex flex-col items-center gap-6 z-30 hidden lg:flex">
        <button
          v-for="section in ['vocabulary', 'conversation', 'examples']"
          :key="section"
      ...
      <div class="max-w-4xl mx-auto w-full lg:pl-12 relative">
        ...
        <div class="space-y-16">
          ...
          <section
            ...
            id="vocabulary"
            class="scroll-mt-48 transition-all duration-500"
          >
            ...
            <div class="grid grid-cols-1 md:grid-cols-1 gap-4">
              <div
                ...
                :key="item.word_id"
                class="bg-white p-4 rounded-xl shadow-sm border-l-4 transition-all duration-300 cursor-pointer hover:shadow-md"
                :class="
                  isHighlighted(item.kanji)
          ...
          >
            ...
            <div class="space-y-4">
              <div
                v-for="ex in examplesData"
                :key="ex.id"
                class="bg-white p-5 rounded-xl shadow-sm border-l-4 transition-all duration-300 cursor-pointer hover:shadow-md"
                :class="
                  matchParts(ex.parts, activeLyricLine)
                    ? 'border-amber-400 bg-amber-50'
                    : 'border-gray-200 hover:border-gray-300'
                "
                @click="playAudioAt(ex.audioTime)"
              >
                <p class="text-xl text-gray-800 mb-2 font-medium leading-relaxed">
                  <template v-for="(part, idx) in ex.parts" :key="idx">
                    <ruby v-if="part.r" class="mx-0.5">
                      {{ part.k }}
                      <rt class="text-xs text-amber-600 font-normal">{{ part.r }}</rt>
                    </ruby>
                    <span v-else class="mx-px">{{ part.k }}</span>
                  </template>
                </p>
                <p class="text-gray-500 text-sm italic">{{ ex.translation }}</p>
              </div>
            </div>
          </section>
          <!-- Conversation Section -->
          <section
            :ref="sectionRefs.conversation"
            id="conversation"
            class="scroll-mt-48 transition-all duration-500"
          >
            <div class="flex items-center gap-4 mb-6 border-b border-gray-200 pb-4">
            ...
            <div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
              ...
              <div class="p-6 space-y-6">
                <div
                  v-for="line in conversationData.lines"
                  :key="line.id"
                  class="flex gap-4 group cursor-pointer p-4 rounded-lg transition-all duration-300 border-l-4"
                  :class="
                    matchParts(line.parts, activeLyricLine)
                      ? 'border-indigo-500 bg-indigo-50 shadow-sm'
                      : 'border-transparent hover:bg-gray-50'
                  "
                  @click="playAudioAt(line.audioTime)"
                >
                  <div
                    class="flex-shrink-0 w-10 h-10 rounded-full bg-indigo-100 text-indigo-600 flex items-center justify-center font-bold text-sm shadow-sm transition-transform duration-300"
                    :class="matchParts(line.parts, activeLyricLine) ? 'scale-110' : ''"
                  >
                    {{ line.speaker }}
                  </div>
                  <div class="flex-1">
                    <p
                      class="text-lg text-gray-800 mb-2 font-medium leading-relaxed transition-colors duration-300"
                      :class="{ 'text-indigo-800': matchParts(line.parts, activeLyricLine) }"
                    >
                      <template v-for="(part, idx) in line.parts" :key="idx">
                        <ruby v-if="part.r" class="mx-0.5">
                          {{ part.k }}
                          <rt class="text-xs text-indigo-500 font-normal">{{ part.r }}</rt>
                        </ruby>
                        <span v-else>{{ part.k }}</span>
                      </template>
                    </p>
                    <p class="text-gray-500 text-sm">{{ line.translation }}</p>
                  </div>
                </div>
              </div>
            </div>
